...
#include <follower_velocity_smoother/paramsConfig.h>

#include <ecl/threads/thread.hpp>

#include "follower_velocity_smoother/velocity_smoother_nodelet.hpp"
...
void VelocitySmoother::reconfigCB(follower_velocity_smoother::paramsConfig &config, uint32_t level)
{
  ROS_INFO("Reconfigure request : %f %f %f %f %f", config.speed_lim_v, config.speed_lim_w, config.accel_lim_v,
...
void VelocitySmoother::velocityCB(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  target_vel.angular.z =
      ...
      ROS_INFO("----=%lf, =%lf;-------------------",target_vel.linear.x, target_vel.angular.z);
}
...
void VelocitySmoother::spin()
{
  ...
  while (!shutdown_req && ros::ok())
  {
        ...
        ((ros::Time::now() - last_cb_time).toSec() > std::min(3.0 * cb_avg_time, 0.5)))
    {
      ...
      {
        ROS_INFO_STREAM("Velocity Smoother : input got inactive leaving us a "
         ...
         (std::abs(current_vel.angular.z - last_cmd_vel.angular.z) > 2.0)))
    {
      ...
      ROS_INFO("Using robot velocity feedback (%s) instead of last command: "
               "%f, %f, %f",
               robot_feedback == ODOMETRY ? "odometry" : "end commands", (ros::Time::now() - last_cb_time).toSec(),
               current_vel.linear.x - last_cmd_vel.linear.x, current_vel.angular.z - last_cmd_vel.angular.z);
      last_cmd_vel = current_vel;
    }

    geometry_msgs::TwistPtr cmd_vel;

    if ((target_vel.linear.x != last_cmd_vel.linear.x) || (target_vel.angular.z != last_cmd_vel.angular.z))
    {
      ...
      // Calculate and normalise vectors A (desired velocity increment) and B
      // (maximum velocity increment),
      // where v acts as coordinate x and w as coordinate y; the sign of the
      // angle from A to B determines
      // which velocity (v or w) must be overconstrained to keep the direction
      // provided as command
      ...
      std::cout << "sm_x:" << cmd_vel->linear.x << std::endl;
...
bool VelocitySmoother::init(ros::NodeHandle &nh)
{
  ...
  dynamic_reconfigure_server = new dynamic_reconfigure::Server<follower_velocity_smoother::paramsConfig>(nh);
  ...
  {
    ROS_INFO("Invalid robot feedback type (%d). Valid options are 0 (NONE, "
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  follower_velocity_smoother::VelocitySmoother vsnv();
